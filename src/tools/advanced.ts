...
import { dialTunnelConnection } from '../tunnel-dial.ts';
import {
  getTempFilename,
  ...
  registerToolConditional(
    ...
    async ({ server, type, localHost, localPort, remoteHost, remotePort }) => {
      try {
        ...
        const ssh = await dialTunnelConnection(resolved, resolveServer);
