...
|---|---|---|---|
| ISSUE-01 | SSH 隧道嵌套跳板 / 真机多跳验证 | 单跳 `proxyJump` / `proxyCommand` 已由隧道自握门卫 Connection 实现（ADR-0003）；门卫自身仍带跳字段时继续拒绝。未把隧道挂进 Connection 池 | 嵌套跳链、与池共用门卫（引用计数）、真实多跳拓扑上的协议/NAT 行为尚未验证 |
