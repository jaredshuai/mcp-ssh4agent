...
> **Update (2026-08-22, npm publish readiness):** the *published npm artifact* is compiled to plain ES2022 JS by `tsc -p tsconfig.build.json` on `prepack` (`rewriteRelativeImportExtensions` rewrites the `.ts` import specifiers), because Node hard-refuses type stripping under `node_modules` — shipping the sources could never work with `npx`. The no-build contract above is unchanged where it always applied: the dev tree still runs `node src/index.ts` natively, and everything in-repo stays build-free. Side effect: consumers of the package need only Node >=20 (verified under 20.20.2 and 22.23.2 via fnm), while developing on the sources still requires >=23.6.
